...
//		//desktop.open(file);
	...
	private void methodTocreateSampleResult() throws Exception {
		...
		sno++;
		Select sampleType = new Select(driver.findElement(By.id("sampleTypeInLimsSampleResultSearch")));
		Thread.sleep(2000);
		sampleType.selectByVisibleText(properties.getProperty("SampleType_Name"));
//		lifeCycle.selectByIndex(1);
		document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select Sample Type", sno, false);
		Thread.sleep(2000);
		sno++;
		driver.findElement(By.id("searchBtnInLimsSampleResult")).click();
