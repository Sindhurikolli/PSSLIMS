...
//		Thread.sleep(3000);
		...
		isRecordSelected1 = selectRecordForProduct( count1,  isRecordSelected1, productCode);
		
		if (isRecordSelected1) {
			driver.findElement(By.id("prodSelBtnIdInSpecReg")).click();
			document = Utilities.getScreenShotAndAddInLogDoc(driver, document, "Select product Details", sno,
					false);
			Thread.sleep(3000);	
			sno++;
		}else {
			System.out.println("Product Record is not Selected");
...
//			Thread.sleep(2000);
	...
	private boolean selectRecordForProduct(int count1, boolean isRecordSelected1, String productCode) throws Exception {
		...
		if (perPageNoOfRecordsPresent > 0 && count1 == 0) {
			if ((totalNoOfRecords > 1) && ((productCode == null) || ("".equalsIgnoreCase(productCode)))) {
				productCode = driver
						.findElement(By.xpath("//*[@id=\"ProSelRegSpecAddWinTable\"]/div/div[4]/table/tbody/tr[1]/td[3]"))
						.getText();
				System.out.println(productCode);
			} else if ((productCode == null) || ("".equalsIgnoreCase(productCode))) {
				productCode = driver.findElement(By.id("//*[@id=\"ProSelRegSpecAddWinTable\"]/div/div[4]/table/tbody/tr[1]/td[3]"))
						.getText();
				System.out.println(productCode);
			}
			++count1;
		...
		if (perPageNoOfRecordsPresent > 0) {
			while (NoOfRecordsChecked < totalNoOfRecords) {
				if (totalNoOfRecords > 1) {
					...
					String productCodeSequence = driver
							.findElement(By.xpath("//*[@id=\"ProSelRegSpecAddWinTable\"]/div/div[4]/table/tbody/tr/td[3]"))
							.getText();
					if (productCode.equalsIgnoreCase(productCodeSequence)) {
						driver.findElement(By.xpath("//*[@id=\"ProSelRegSpecAddWinTable\"]/div/div[4]/table/tbody/tr/td[3]"))
								.click();
						isRecordSelected1 = true;
						break;
					}
